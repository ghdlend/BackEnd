...
class C extends B{
	...
	//생성자 오버로딩
	public C(int x) {
		//super 메서드는 첫 행에 기술해야 함 this()도 마찬가지로 첫 행에 기술
		super(x); //B클래스의 매개변수가 있는 생성자 선택 - 명시적 생성자 호출
		System.out.println("매개변수 생성자 C : " + x);
	}
	public void tmp() {
		//super(x); //일반메서드에서는 사용 불가, 생성자 내에서만 사용 가능
	}
}
...
public class SuperConstTest {
	...
	public static void main(String[] args) {
		...
		//C클래스 인스턴스 생성 - 매개변수가 있는 생성자 메서드 사용
		C c1 = new C(5);//명시적으로 부모 클래스 생성자 호출하지 않음, 매개변수가 없는 기본 생성자가 자동 호출 됨
	}
